...
namespace Trustlink.UnitTests.IO
{
    [TestClass]
    public class UT_IOHelper
    {
        ...
        public void TestAsSerializableGeneric()
        {
            byte[] caseArray = new byte[] { 0x00,0x00,0x00,0x00,0x00,
                                            ...
                                            0x00,0x00,0x00,0x00,0x00,
                                            0x00,0x00,0x00,0x00,0x00 };
            UInt160 result = Trustlink.IO.Helper.AsSerializable<UInt160>(caseArray);
            ...
            for (int i = 0; i < 2; i++)
            {
                if (i == 0)
                {
                    byte[] caseArray = new byte[] { 0x00,0x00,0x00,0x00,0x00,
                                                    ...
                                                    0x00,0x00,0x00,0x00,0x00,
                                                    0x00,0x00,0x00,0x00,0x00 };
                    ISerializable result = Trustlink.IO.Helper.AsSerializable(caseArray, typeof(UInt160));
                    Assert.AreEqual(UInt160.Zero, result);
                }
                else
                {
                    Action action = () => Trustlink.IO.Helper.AsSerializable(new byte[0], typeof(Double));
        ...
        {
            byte[] byteArray = Trustlink.IO.Helper.ToByteArray<UInt160>(new UInt160[] { UInt160.Zero });
            UInt160[] result = Trustlink.IO.Helper.AsSerializableArray<UInt160>(byteArray);
        ...
        public void TestGetVarSizeInt()
        {
            for (int i = 0; i < 3; i++)
            {
                if (i == 0)
                {
                    int result = Trustlink.IO.Helper.GetVarSize(1);
                    Assert.AreEqual(1, result);
                }
                else if (i == 1)
                {
                    int result = Trustlink.IO.Helper.GetVarSize(0xFFFF);
                    Assert.AreEqual(3, result);
                }
                else
                {
                    int result = Trustlink.IO.Helper.GetVarSize(0xFFFFFF);
        ...
        public void TestGetVarSizeGeneric()
        {
            for (int i = 0; i < 9; i++)
            {
                if (i == 0)
                {
                    int result = Trustlink.IO.Helper.GetVarSize(new UInt160[] { UInt160.Zero });
                ...
                else if (i == 1)//sbyte
                {
                    ...
                    int result = Trustlink.IO.Helper.GetVarSize(testList);
                ...
                else if (i == 8)
                {
                    ...
                    int result = Trustlink.IO.Helper.GetVarSize<int>(testList);
                    Assert.AreEqual(5, result);
                }
            }
        }
        ...
        {
            int result = Trustlink.IO.Helper.GetVarSize("AA");
            Assert.AreEqual(3, result);
        }
        ...
        public void TestReadBytesWithGrouping()
        {
            for (int i = 0; i < 2; i++)
            {
                if (i == 0)
                {
                    ...
                    Trustlink.IO.Helper.WriteBytesWithGrouping(writer, caseArray);
                    stream.Seek(0, SeekOrigin.Begin);
                    BinaryReader reader = new BinaryReader(stream);
                    byte[] result = Trustlink.IO.Helper.ReadBytesWithGrouping(reader);
                ...
                else
                {
                    ...
                    stream.Seek(0, SeekOrigin.Begin);
                    BinaryReader reader = new BinaryReader(stream);
                    Action action = () => Trustlink.IO.Helper.ReadBytesWithGrouping(reader);
        ...
        public void TestReadFixedString()
        {
            MemoryStream stream = new MemoryStream();
            BinaryWriter writer = new BinaryWriter(stream);
            Trustlink.IO.Helper.WriteFixedString(writer, "AA", Encoding.UTF8.GetBytes("AA").Length + 1);
            stream.Seek(0, SeekOrigin.Begin);
            BinaryReader reader = new BinaryReader(stream);
            string result = Trustlink.IO.Helper.ReadFixedString(reader, Encoding.UTF8.GetBytes("AA").Length + 1);
        ...
        public void TestReadSerializable()
        {
            MemoryStream stream = new MemoryStream();
            BinaryWriter writer = new BinaryWriter(stream);
            Trustlink.IO.Helper.Write(writer, UInt160.Zero);
            stream.Seek(0, SeekOrigin.Begin);
            BinaryReader reader = new BinaryReader(stream);
            UInt160 result = Trustlink.IO.Helper.ReadSerializable<UInt160>(reader);
            Assert.AreEqual(UInt160.Zero, result);
        }
        ...
        public void TestReadSerializableArray()
        {
            MemoryStream stream = new MemoryStream();
            BinaryWriter writer = new BinaryWriter(stream);
            Trustlink.IO.Helper.Write(writer, new UInt160[] { UInt160.Zero });
            stream.Seek(0, SeekOrigin.Begin);
            BinaryReader reader = new BinaryReader(stream);
            UInt160[] resultArray = Trustlink.IO.Helper.ReadSerializableArray<UInt160>(reader);
        ...
        public void TestReadVarBytes()
        {
            MemoryStream stream = new MemoryStream();
            BinaryWriter writer = new BinaryWriter(stream);
            Trustlink.IO.Helper.WriteVarBytes(writer, new byte[] { 0xAA, 0xAA });
            stream.Seek(0, SeekOrigin.Begin);
            BinaryReader reader = new BinaryReader(stream);
            byte[] byteArray = Trustlink.IO.Helper.ReadVarBytes(reader, 10);
        ...
        public void TestReadVarInt()
        {
            for (int i = 0; i < 4; i++)
            {
                if (i == 0)
                {
                    MemoryStream stream = new MemoryStream();
                    BinaryWriter writer = new BinaryWriter(stream);
                    Trustlink.IO.Helper.WriteVarInt(writer, 0xFFFF);
                    stream.Seek(0, SeekOrigin.Begin);
                    BinaryReader reader = new BinaryReader(stream);
                    ulong result = Trustlink.IO.Helper.ReadVarInt(reader, 0xFFFF);
                ...
                else if (i == 1)
                {
                    MemoryStream stream = new MemoryStream();
                    BinaryWriter writer = new BinaryWriter(stream);
                    Trustlink.IO.Helper.WriteVarInt(writer, 0xFFFFFFFF);
                    stream.Seek(0, SeekOrigin.Begin);
                    BinaryReader reader = new BinaryReader(stream);
                    ulong result = Trustlink.IO.Helper.ReadVarInt(reader, 0xFFFFFFFF);
                ...
                else
                {
                    MemoryStream stream = new MemoryStream();
                    BinaryWriter writer = new BinaryWriter(stream);
                    Trustlink.IO.Helper.WriteVarInt(writer, 0xFFFFFFFFFF);
                    stream.Seek(0, SeekOrigin.Begin);
                    BinaryReader reader = new BinaryReader(stream);
                    Action action = () => Trustlink.IO.Helper.ReadVarInt(reader, 0xFFFFFFFF);
                    action.ShouldThrow<FormatException>();
                }
            }
        }
        ...
        public void TestReadVarString()
        {
            MemoryStream stream = new MemoryStream();
            BinaryWriter writer = new BinaryWriter(stream);
            Trustlink.IO.Helper.WriteVarString(writer, "AAAAAAA");
            stream.Seek(0, SeekOrigin.Begin);
            BinaryReader reader = new BinaryReader(stream);
            string result = Trustlink.IO.Helper.ReadVarString(reader, 10);
            stream.Seek(0, SeekOrigin.Begin);
        ...
        {
            byte[] byteArray = Trustlink.IO.Helper.ToArray(UInt160.Zero);
        ...
        {
            byte[] byteArray = Trustlink.IO.Helper.ToByteArray(new UInt160[] { UInt160.Zero });
        ...
        public void TestWriteBytesWithGrouping()
        {
            MemoryStream stream = new MemoryStream();
            BinaryWriter writer = new BinaryWriter(stream);
            Trustlink.IO.Helper.WriteBytesWithGrouping(writer, new byte[] { 0xAA, 0xAA, 0xAA, 0xAA, 0xAA, 0xAA, 0xAA, 0xAA,
                                                                      0xAA, 0xAA, 0xAA, 0xAA, 0xAA, 0xAA, 0xAA, 0xAA,
                                                                      0xAA, 0xAA, 0xAA, 0xAA, 0xAA, 0xAA, 0xAA, 0xAA,
                                                                      0xAA, 0xAA, 0xAA, 0xAA, 0xAA, 0xAA, 0xAA, 0xAA,
                                                                      0xAA, 0xAA, 0xAA, 0xAA});
        ...
        public void TestWriteFixedString()
        {
            for (int i = 0; i < 5; i++)
            {
                if (i == 0)
                {
                    MemoryStream stream = new MemoryStream();
                    BinaryWriter writer = new BinaryWriter(stream);
                    Action action = () => Trustlink.IO.Helper.WriteFixedString(writer, null, 0);
                ...
                else if (i == 1)
                {
                    MemoryStream stream = new MemoryStream();
                    BinaryWriter writer = new BinaryWriter(stream);
                    Action action = () => Trustlink.IO.Helper.WriteFixedString(writer, "AA", Encoding.UTF8.GetBytes("AA").Length - 1);
                ...
                else if (i == 2)
                {
                    MemoryStream stream = new MemoryStream();
                    BinaryWriter writer = new BinaryWriter(stream);
                    Action action = () => Trustlink.IO.Helper.WriteFixedString(writer, "拉拉", Encoding.UTF8.GetBytes("拉拉").Length - 1);
                    action.ShouldThrow<ArgumentException>();
                }
                else if (i == 3)
                {
        ...
        public void TestWriteVarBytes()
        {
            MemoryStream stream = new MemoryStream();
            BinaryWriter writer = new BinaryWriter(stream);
            Trustlink.IO.Helper.WriteVarBytes(writer, new byte[] { 0xAA });
            stream.Seek(0, SeekOrigin.Begin);
            byte[] byteArray = new byte[stream.Length];
            stream.Read(byteArray, 0, (int)stream.Length);
        ...
        public void TestWriteVarInt()
        {
            for (int i = 0; i < 5; i++)
            {
                if (i == 0)
                {
                    MemoryStream stream = new MemoryStream();
                    BinaryWriter writer = new BinaryWriter(stream);
                    Action action = () => Trustlink.IO.Helper.WriteVarInt(writer, -1);
                ...
                else if (i == 1)
                {
                    MemoryStream stream = new MemoryStream();
                    BinaryWriter writer = new BinaryWriter(stream);
                    Trustlink.IO.Helper.WriteVarInt(writer, 0xFC);
                    stream.Seek(0, SeekOrigin.Begin);
                    byte[] byteArray = new byte[stream.Length];
                    stream.Read(byteArray, 0, (int)stream.Length);
                ...
                else
                {
                    MemoryStream stream = new MemoryStream();
                    BinaryWriter writer = new BinaryWriter(stream);
                    Trustlink.IO.Helper.WriteVarInt(writer, 0xAEFFFFFFFF);
                    stream.Seek(0, SeekOrigin.Begin);
                    byte[] byteArray = new byte[stream.Length];
                    stream.Read(byteArray, 0, (int)stream.Length);
        ...
        public void TestWriteVarString()
        {
            MemoryStream stream = new MemoryStream();
            BinaryWriter writer = new BinaryWriter(stream);
            Trustlink.IO.Helper.WriteVarString(writer, "a");
            stream.Seek(0, SeekOrigin.Begin);
            byte[] byteArray = new byte[stream.Length];
            stream.Read(byteArray, 0, (int)stream.Length);
