...
using Trustlink.VM;
using ECPoint = Trustlink.Cryptography.ECC.ECPoint;
...
namespace Trustlink.Wallets
{
    public abstract class Wallet
    {
        ...
        public BigDecimal GetBalance(UInt160 asset_id, params UInt160[] accounts)
        {
            ...
            ApplicationEngine engine = ApplicationEngine.Run(script, extraLINK: 20000000L * accounts.Length);
        ...
        public static byte[] GetPrivateKeyFromTLP2(string nep2, string passphrase, int N = 16384, int r = 8, int p = 8)
        {
        ...
        public virtual WalletAccount Import(string tlp2, string passphrase, int N = 16384, int r = 8, int p = 8)
        {
            byte[] privateKey = GetPrivateKeyFromTLP2(tlp2, passphrase, N, r, p);
            WalletAccount account = CreateAccount(privateKey);
            Array.Clear(privateKey, 0, privateKey.Length);
            return account;
        }
            ...
            using (Snapshot snapshot = Blockchain.Singleton.GetSnapshot())
            {
                ...
                using (ScriptBuilder sb = new ScriptBuilder())
                {
                    foreach (var (assetId, group, sum) in outputs.GroupBy(p => p.AssetId, (k, g) => (k, g, g.Select(p => p.Value.Value).Sum())))
                    {
                        ...
                        if (assetId.Equals(NativeContract.LINK.Hash))
                            balances_gas = balances;
                    }
                    script = sb.ToArray();
                }
                if (balances_gas is null)
                    balances_gas = accounts.Select(p => (Account: p, Value: NativeContract.LINK.BalanceOf(snapshot, p))).Where(p => p.Value.Sign > 0).ToList();
        ...
        public Transaction MakeTransaction(byte[] script, UInt160 sender = null, TransactionAttribute[] attributes = null, Cosigner[] cosigners = null)
        {
            ...
            using (Snapshot snapshot = Blockchain.Singleton.GetSnapshot())
            {
                var balances_gas = accounts.Select(p => (Account: p, Value: NativeContract.LINK.BalanceOf(snapshot, p))).Where(p => p.Value.Sign > 0).ToList();
        ...
        private Transaction MakeTransaction(Snapshot snapshot, byte[] script, TransactionAttribute[] attributes, Cosigner[] cosigners, List<(UInt160 Account, BigInteger Value)> balances_gas)
        {
            ...
            throw new InvalidOperationException("Insufficient LINK");
        }
